...
fn valid_nested_empty() {
    assert!(handle_file("test-data/step4/valid.json").is_ok());
}

#[test]
fn valid_nested_with_values() {
    assert!(handle_file("test-data/step4/valid2.json").is_ok());
}

#[test]
fn invalid_single_quoted_array() {
    let result = handle_file("test-data/step4/invalid.json");
